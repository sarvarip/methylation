...
-awk '{ for(i=2;i<=10;i++) {printf "%s ", $i} printf "\n"}' methylation.txt | awk '{for (i=1;i<=NF;i++) {if (NR==1) name[i] = $i; else total[i]+=$i; sq[i]+=$i*$i}} END {for(i=1;i<=NF;i++) {print name[i], sq[i]/NR-(total[i]/NR)**2}}'
- for i in {2..866092}; do echo $i.variance; done | xargs snakemake -p -s methcounts.smk -j 30 --cluster "{params.grid_opts}" --rerun-incomplete --latency-wait 60
